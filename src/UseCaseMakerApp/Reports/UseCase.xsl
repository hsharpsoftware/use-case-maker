...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:param name="triggerEventNodeSet" />
  ...
  <xsl:template match="EventType">
    ...
    <xsl:variable name="value" select="$triggerEventNodeSet[@EnumName = $target]" />
    <tr>
      <td class="TableCell">
		...
		<table align="center" class="Table" cellpadding="2" cellspacing="0" width="600px">
			...
			<tr>
				<td class="TableCell" colspan="2">
					<xsl:variable name="uid" select="ActorUniqueID/text()" />
          <a class="href_elementname" href="{$uid}.htm" target="mainPage">
            <xsl:value-of select="//Actor[@UniqueID = $uid]/@Name" />
          </a>
