    ...
    <xsl:param name="text" />
     	<xsl:choose>
    		<xsl:when test="contains($text,'&#34;') and string-length($text) > 1">
					...
					<xsl:when test="$end">
						...
						<xsl:choose>
							<xsl:when test="//Glossary/*[@Name = $end]">
								<xsl:call-template name="MakeGlossaryLink">
              ...
              <xsl:when test="//Stakeholders/*[@Name = $end]">
                <xsl:call-template name="MakeStakeholderLink">
                  <xsl:with-param name="text" select="$end" />
                  <xsl:with-param name="node" select="//Stakeholders/*[@Name = $end]" />
                </xsl:call-template>
                <xsl:call-template name="MatchLink">
                  <xsl:with-param name="text" select="substring-after($text,concat($end,'&#34;'))" />
                </xsl:call-template>
...
<xsl:template name="MakeGlossaryLink">
	<xsl:param name="text" />
	<xsl:param name="node" />
	
	<xsl:variable name="target" select="concat('Glossary','.htm','#',$node/@UniqueID)" />
...
<xsl:template name="MakeStakeholderLink">
  <xsl:param name="text" />
  <xsl:param name="node" />

  <xsl:variable name="target" select="concat('Stakeholders','.htm','#',$node/@UniqueID)" />
  <a class="href_stakeholder" href="{$target}">
    <xsl:value-of select="$text" />
  </a>
  <xsl:value-of select="'&#34;'" />
</xsl:template>  
